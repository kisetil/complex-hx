...
abstract Complex(ComplexType) from ComplexType to ComplexType {
	...
	**/
	public static inline function fromPolar(p:PolarCoordinates) {
		return new Complex(p.magnitude * Math.cos(p.phaseAngle), p.magnitude * Math.sin(p.phaseAngle));
	}

	public static inline function toPolar(z:Complex):PolarCoordinates {
		return new PolarCoordinates(abs(z), arg(z));
	}
	...
	**/
	...
	public static inline function sqrt(z:Complex):Complex {
		...
		return Complex.fromPolar(new PolarCoordinates(m, a));
	}

	public static inline function exp(z:Complex):Complex {
		var m = Math.exp(z.real);
		return Complex.fromPolar(new PolarCoordinates(m, z.imag));
	}
	...
	public static inline function pow(base:Complex, exponent:Float):Complex {
		...
		return Complex.fromPolar(new PolarCoordinates(nM, nA));
	}
